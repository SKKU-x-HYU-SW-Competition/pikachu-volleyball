...
- **`develop`**: 통합 브랜치. 스킬 트랙을 제외한 모든 feature 브랜치는 여기서 분기하고, 작업이
  ...
  브랜치입니다.
  - **스킬 시스템은 게이지(Phase 4-A)까지만 `develop`에 넣습니다.** 게이지는 스킬 효과와 무관하게
    동작하는 독립 자원 시스템이고 다른 트랙(테스트 환경·제출/중계)과 충돌하지 않으므로 통합
    브랜치에 둡니다.
- **`develop-skill`**: 스킬 트랙 전용 통합 브랜치. 게이지 이후의 **스킬 효과·발동·밸런싱 작업은
  전부 여기서** 진행합니다 (팀장 결정, 2026-08-10).
