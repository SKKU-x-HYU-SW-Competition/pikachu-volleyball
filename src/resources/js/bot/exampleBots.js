/**
 * Example bot source, used only by devBotHook.js for manual Phase 1
 * smoke-testing (see docs/agent-dev/PHASES.md Phase 1). Not part of the
 * protocol spec -- just a minimal "does the plumbing work" demonstration:
 * chase the ball's expected landing point and jump/smash when close.
 *
 * This is plain source text (not a real ES module) because it's sent to
 * botWorker.js as a string and evaluated there, exactly like a
 * participant's submitted bot would be (see botWorker.js's `init` handler).
 */
'use strict';
