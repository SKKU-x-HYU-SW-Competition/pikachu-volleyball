...
학습 포인트:
    - snapshot['side']를 사용하면 하나의 코드로 LEFT와 RIGHT 양쪽에서 동작할 수 있습니다.
    - ball['expectedLandingPointX']로 공이 떨어지기 전에 수비 위치를 정할 수 있습니다.
    - self['state']와 공의 속도를 함께 보면 불필요한 점프를 줄일 수 있습니다.
...
JUMP_HORIZONTAL_RANGE = 32
JUMP_MAX_HORIZONTAL_SPEED = 5
JUMP_MAX_BALL_Y = 150


# 예상 착지점이 현재 봇이 지키는 코트에 포함되는지 확인합니다.
# 이 판단 덕분에 공이 상대 코트에 있을 때 불필요하게 네트까지 쫓아가지 않습니다.
def is_expected_landing_on_own_side(side, expected_landing_point_x):
    if side == 'LEFT':
        return 0 <= expected_landing_point_x <= NET_X
    return NET_X <= expected_landing_point_x <= 432
...
def get_target_x(snapshot):
    side = snapshot['side']
    landing_x = snapshot['ball']['expectedLandingPointX']

    if not is_expected_landing_on_own_side(side, landing_x):
        # 공이 상대 코트에 떨어질 예정이면 자기 코트 중앙으로 복귀합니다.
        return LEFT_STANDBY_X if side == 'LEFT' else RIGHT_STANDBY_X

    # 공이 자기 코트에 떨어질 예정이면 착지점 바로 아래가 아니라 네트의
    # 반대쪽으로 12px 물러난 위치를 목표로 삼습니다.
    if side == 'LEFT':
        return clamp(landing_x - LANDING_OFFSET, LEFT_MIN_X, LEFT_MAX_X)
    return clamp(landing_x + LANDING_OFFSET, RIGHT_MIN_X, RIGHT_MAX_X)


# 목표까지의 차이가 정지 구간보다 클 때만 한 방향의 이동 액션을 냅니다.
def move_toward(current_x, target_x):
    if target_x < current_x - MOVE_DEAD_ZONE:
        return -1
    if target_x > current_x + MOVE_DEAD_ZONE:
        return 1
    return 0
...
def should_jump(snapshot):
    ...
    return (
        ...
        and abs(ball['x'] - player['x']) < JUMP_HORIZONTAL_RANGE
        # 옆으로 너무 빠른 공은 점프하는 동안 위치가 크게 달라지므로 포기합니다.
        and abs(ball['xVelocity']) < JUMP_MAX_HORIZONTAL_SPEED
        # y좌표는 아래로 갈수록 커집니다. 150보다 작은 공은 충분히 높은 공입니다.
        and ball['y'] < JUMP_MAX_BALL_Y
...
def decide(snapshot):
    ...
    target_x = get_target_x(snapshot)
    # 2단계: 목표를 향한 좌우 이동 액션을 만듭니다.
    x = move_toward(snapshot['self']['x'], target_x)
    # 3단계: 모든 안전 조건을 만족할 때만 y=-1로 점프합니다.
    y = -1 if should_jump(snapshot) else 0
