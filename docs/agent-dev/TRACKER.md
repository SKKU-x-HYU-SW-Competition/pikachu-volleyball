...
|---|---|---|---|---|---|
| 2026-08-10 | (커밋 전) | 팀장 imtmtl | ④ 스킬/에셋 | Phase 4-A PR 준비 — 코드 변경 없음, 검증만 재실행. `npm run verify`
  통과(남는 경고는 pyodide/에셋 크기 관련 기존 항목뿐). `GaugeTracker` 순수 로직을 14케이스로 재확인(서브 +0 / 1~4터치 산술 / 상하한
  클램프 / 랠리 경계 후 첫 접촉이 서브로 판정 / `startOfNewGame` **진입 전이에서만** 리셋 / 접촉 플래그가 여러 틱 유지돼도 1회만
  카운트 / `round()` 밖 접촉 무시 / 양측 동시 접촉). **브라우저 검증을 "육안 확인"에서 실측으로 승격**: Playwright로 좌우 모두 기본
  AI로 두고 자동 랠리를 돌린 뒤 캔버스 픽셀에서 게이지 바 길이를 직접 측정 — 두 바가 5 단위로 증감하고, 경기 시작 시 0으로 리셋되며,
  콘솔 에러 0인 것을 확인. 계약 적합성: `src/resources/js/bot/`에 `gauge` 문자열이 하나도 없어 봇 프로토콜 미변경 확인 → CONTRACTS.md
  변경 없음. 앞선 08-08 행의 실제 커밋 해시는 `35e1360`, `0c776cf` | ADR-0019 변경 없음(OPEN 5건 그대로 회의 안건). CONTRACTS.md 변경
  없음. Issue #5 |
| 2026-08-08 | (커밋 전) | 팀장 imtmtl | ④ 스킬/에셋 | Phase 4 착수 — 스킬의 자원이 될 **게이지**만 먼저 구현(스킬 효과·봇 프로토콜은 다음 PR로 분리). 충전 규칙은 "상대에게서 넘어온 공을 처음 받으면 +10 / 자기 진영에서 이어 치면 -5 / 서브는 +0", 범위 0~100 클램프, 리셋은 경기 시작 시에만. 서브를 0으로 둔 건 원작이 득점자에게 서브권을 주기 때문(눈덩이 방지) — 서브 규칙이 바뀌면 `GAUGE_ON_SERVE` 상수 하나만 수정. **엔진 파일 무수정 원칙 유지**: `physics.js`에 필드를 넣는 대신 `player.isCollisionWithBallHappened`의 false→true 전이를 매 틱 밖에서 관찰하는 방식(`bot/testSetup.js`와 같은 패턴)이라, 05번 문서 분류상 Tier B가 아닌 Tier A로 구현됨. 신규 `skill/gauge.js`(순수 로직) / `skill/gaugeView.js`(점수판 아래 막대 2개, 스프라이트시트 단색 타일 stretch) / `skill/setup.js`(ticker+GameView 연결), `main.js` 2줄. `npm run verify` 통과, `GaugeTracker` 로직 15개 케이스 임시 스크립트로 검증(서브/1~4터치/클램프/랠리 경계/경기 리셋/접촉 중복 카운트 방지), 브라우저에서 렌더링 육안 확인. 확인 과정에서 "AI vs AI일 때 파워히트 키로 인트로 복귀"는 원작 동작(`pikavolley.js:331-344`)임을 확인 — 이번 변경과 무관 | ADR-0019 신규(PARTIAL — 충전 규칙만 확정, 스킬 효과/소모량/봇 프로토콜/사람 발동키/게이지 눈금은 OPEN), DECISIONS.md 인덱스, `docs/architecture/05-skill-extension-guide.md` 갱신 노트. CONTRACTS.md 변경 없음(봇 프로토콜 미변경) |
