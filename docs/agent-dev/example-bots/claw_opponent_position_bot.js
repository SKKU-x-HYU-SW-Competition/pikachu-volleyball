/**
 * Example bot: claw aimed at the OPPONENT'S CURRENT POSITION.
 *
 * Paired with claw_landing_point_bot.js as a controlled experiment. The two
 * files are byte-identical except for aimX() at the bottom -- same
 ...
 *
 * This one aims at the opponent where they stand at cast time -- the obvious
 * reading of "drop it on their head", and the same rule the keyboard uses
 * when a human presses the cast key (§1.1). It is the honest baseline: a
 * claw takes config.claw.warningFrames to land (25 frames = 1.00s), so this
 * only connects when the opponent has no reason to move -- which is exactly
 * what the pairing is meant to measure.
 *
 * Works on either side (reads s.side itself) -- paste into the LEFT or RIGHT
 * textarea in the Bot Setup panel.
 *
...
// ===========================================================================
// SHARED -- keep identical to claw_landing_point_bot.js
// ===========================================================================
...
/**
 * Aim at the opponent's position as of this snapshot.
 *
 * No lead at all: opp.x is where they stood roughly one tick ago, and the
 * claw needs another warningFrames on top of that. Anything that makes them
 * move in the meantime -- chasing a receive, dodging, a smash follow-through
 * -- is a miss. Kept deliberately naive; this is the control, not an attempt
 * at a good bot.
 */
function aimX(s) {
  return s.opp.x;
