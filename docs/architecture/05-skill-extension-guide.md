...
> 후순위, 착수 전). 이 문서는
>
> **2026-08-08 갱신**: 스킬의 자원이 될 **게이지**만 먼저 구현됐습니다
> ([ADR-0019](../agent-dev/decisions/ADR-0019-gauge-system.md), `src/resources/js/skill/`). 스킬 효과
> 자체와 봇 프로토콜 확장은 여전히 미착수 — 즉 아래 5절의 Tier A/B 논의는 그대로 유효합니다.
> 참고로 게이지는 엔진 밖에서 `player.isCollisionWithBallHappened`를 매 틱 관찰하는 방식이라
> **Tier A로 구현됐고, 엔진 파일 무수정 원칙은 아직 깨지지 않았습니다**. (1) 우리 엔진에서 스킬을 넣을 수 있는 지점을 복습하고, (2) 원본
