...
> ([ADR-0020](../agent-dev/decisions/ADR-0020-gauge-system.md), `src/resources/js/skill/`). 스킬 효과
> 자체와 봇 프로토콜 확장은 여전히 미착수라 아래 5절의 Tier A/B 논의는 그대로 유효합니다. 게이지는
> 엔진 밖에서 `player.isCollisionWithBallHappened`를 매 틱 관찰하는 방식이라 **Tier A로 구현됐고,
> 엔진 파일 무수정 원칙은 아직 깨지지 않았습니다**.
